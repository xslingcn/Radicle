...
@ray.remote
class DecodingActor:
    def __init__(self, config: DecoderConfig, ref: "ray.actor.ActorHandle", model: str, tokenizer: str, controller: "ray.actor.ActorHandle"):
        self.ref = ref
    ...
    def _step(self):
        ...
        try:
            batch_kv = torch.stack([
                self.kv_caches[req.request_id] 
                for req in batch
            ])
            ...
            for i, request in enumerate(batch):
                ...
                stream_state = self.stream_states.get(
                    request.request_id, 
                    StreamState()
                )
                ...
                is_complete = (
                    next_token == self.tokenizer.eos_token or 
                    stream_state.total_tokens >= request.max_new_tokens
                )
